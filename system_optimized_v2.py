...
class OptimizedAttendanceSystem:
    ...
    def record_attendance(self, employee_id, timestamp, reader_no=1, verify_method="huella"):
        ...
        try:
            ...
            # Forzar actualización de estado de breaks
            socketio.emit('breaks_update', self.get_break_status())
    ...
    def get_break_status(self):
        """Obtener estado actual de breaks y almuerzos optimizado"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            # Empleados actualmente en break/almuerzo con último evento
            if self.db_type == 'postgresql':
                cursor.execute('''
                    WITH last_events AS (
                        SELECT DISTINCT ON (ar.employee_id) 
                               ar.employee_id, e.name, e.department, ar.event_type, 
                               ar.timestamp, ar.break_type, ar.is_break_record
                        FROM attendance_records ar
                        JOIN employees e ON ar.employee_id = e.employee_id
                        WHERE DATE(ar.timestamp) = %s AND e.active = true
            ...
            return {
                'on_break': on_break,
                'on_lunch': on_lunch,
                'breaks_completed': 0,  # Simplificado para velocidad
                'breaks_pending': 0,
                'lunch_completed': 0,
                'lunch_pending': 0
            }
            ...
            print(f"Error obteniendo estado breaks: {e}")
            return {
                'on_break': [],
                'on_lunch': [],
                'breaks_completed': 0,
